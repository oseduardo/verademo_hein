#/bin/bash
        ...
        #Get Build ID
        build_id=$(cat $OUTPUT_FILE_FULL_PATH | grep "analysis id" | awk -F "\"" '{print $2}')
        echo ""
        echo '====== DEBUG START ======'
        ...
        do
             ...
             then
               ...
               java -jar $JAVA_WRAPPER_LOCATION/VeracodeJavaAPI.jar -vid $1 -vkey $2 -action getbuildinfo -appid $app_ID -sandboxid $sandbox_ID > $OUTPUT_FILE_FULL_PATH 2>&1
             fi
             ...
             then
                   ...
                   then
                        ...
                        java -jar $JAVA_WRAPPER_LOCATION/VeracodeJavaAPI.jar -vid $1 -vkey $2 -action GetPreScanResults -appid $app_ID -sandboxid $sandbox_ID > $OUTPUT_FILE_FULL_PATH 2>&1
                   fi
        ...
        if [ -z "$sandbox_ID" ];
             then
                  ...
                  then
                       ...
                       echo ""
                       java -jar $JAVA_WRAPPER_LOCATION/VeracodeJavaAPI.jar -vid $1 -vkey $2 -action summaryreport -buildid $build_id -outputfilepath $OUTPUT_FILE_FULL_PATH"_SummaryReport.xml"
                       echo "[INFO] See Summary Report in " $OUTPUT_FILE_FULL_PATH"_SummaryReport.xml file. For detailed report, please go to Veracode platform or download Detailed Report by using APIs"
                       echo ""
                       exit 1
                  else
